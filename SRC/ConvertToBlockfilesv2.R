...
RDSfiles<-c("~/BoardAnalytics/TheGreatOverhaul/BoardAnalyticsv2/Results")
...
HeadingCode<-ifelse(Organization=="Atikokan -ON", "\\n *\\d{1,2}\\. *\\d*",
  ifelse(Organization=="KingstonHSC -ON","(\\n\\t|\\n)\\d{1,2}\\.\\d*",
    ifelse(Organization=="SunnyBrook -ON","(\\n\\t|\\n|\\n *)\\d{1,2}\\.\\d*",
    ifelse(Organization=="Barrie -ON","(\\n\\t|\\n|\\n *|\\r)\\d{1,2}\\.\\d*",
    ifelse(Organization=="SickKids -ON","\\n {0,2}\\b[A-Z'-]{2,}\\b *\\b[A-Z]{0,}\\b *\\b[A-Z]{0,}\\b *\\b[A-Z]{0,}\\b *(\\r|\\n)",
    ifelse(Organization=="WilliamOsler -ON","(\\n *|\\n)\\d{1,2}\\.\\d{0,2}",
    ifelse(Organization=="CambridgeMemorial -ON","(\\n|\\r)\\d{1,2}\\.\\d{0,2}\\.{0,1}\\d*",
    ifelse(Organization=="Dryden-ON","\\n *\\d{1,2}\\.\\d{0,2}",
    ifelse(Organization=="HaliburtonHighlands -ON","(\\n\\t|\\n|\\r|\\r\\t)\\d{1,2}\\.\\d{0,2}",
    ifelse(Organization=="Aliston -ON","(\\n\\t|\\n {1,3}|\\n|\\r|\\r\\t)\\d{1,2}\\.\\d{0,2}\\.{0,1}\\d*",
    ifelse(Organization=="LakeridgeHealth -ON", "\\n\\t{0,1}\\d{1,2}\\.\\d{0,2}",
     ifelse(Organization=="SouthBruceGrey -ON", "\\s\\d{1,2}\\.\\d{0,2}\\d(?!%)",
    ifelse(Organization=="UHN -ON", "\\n\\t{0,1}\\d{1,2}\\.\\d{0,2}",
    ifelse(Organization=="WilliamOsler -ON", "(\\r\\t{0,1}|\\n\\t{0,1})\\d{1,2}\\.\\d{0,2}",
    ifelse(Organization=="MSH -ON",{
           "\\n\\t{0,1}\\d{1,2}\\.\\d{0,2}"},
    ifelse(Organization=="PerthSmithFalls -ON", {TEXTBLOCK<-str_replace_all(TEXTBLOCK,"0.\\. *McKenna","O\\. McKenna")
                                                  DBFileList[i,"text"]<-TEXTBLOCK
                                                  "\\t{0,1}\\n\\d{1,2}\\.\\d{0,2}"
                                               },
    ifelse(Organization=="SinaiHealth -ON -ON", "\\n\\d{1,2}\\.\\d{0,2}",
...
BlockFilesFinal<-BlockFiles %>%
 mutate(Heading=ifelse(Organization=="SickKids -ON",paste(SectionIDNumber-1,".0",sep=""),Heading)) %>%
