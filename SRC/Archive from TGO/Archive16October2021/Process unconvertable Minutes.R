...
i<-1
HospitalName<-"Kingston"
DBSource<-"~/BoardAnalytics/TheGreatOverhaul/SourceData/CombinedMinutes/KinstonHSC"
...
doc<-pdf_ocr_text(NHSFiles[i,1])
 ...
 # doc<-iconv(doc,"utf-8","ASCII")
  ...
  write_lines(doc,file.path(DBSource,paste("KHSC -",basename(NHSFiles[i,1]),".csv",sep="")))
